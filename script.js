...
let keys = {};
const types = ['HorizontalPlatform', 'VerticalPlatform'];
...
document.addEventListener('keydown', (event) => {
    if (event.code === 'Semicolon' && player.nearPlatform && player.platformMessage) {
        const platformClass = platformClasses[player.platformMessage.type];
        if (platformClass) {
            const platformCode = platformClass.toString();
            messageBox.innerHTML = `<pre><code class="language-js">${platformCode}</code></pre>`;
            Prism.highlightAll(); // Optional: For syntax highlighting
        }
    }
});
...
document.getElementById('create-class-btn').addEventListener('click', () => {
    ...
    if (!newName) {
        alert("Please enter a name for the new platform class.");
        return;
    }
    ...
    console.log('new platform class')
    console.log(NewPlatformClass)

    console.log(new NewPlatformClass())
    ...
    console.log(`New platform class "${newName}" created with the following action method:`);
...
function createNewPlatformSubclass(name, actionCode) {
    ...
    console.warn(`New platform subclass "${name}" created and added to types.`);
...
class Player {
    ...
    //     setActiveButton(newPlatform.type);  // Update the active button
    // }
...
class Platform {
    ...
    update(player) {
        if (this.isPlayerOnPlatform(player) && keys['KeyL']) {
            this.action(player);
        }
...
class HorizontalPlatform extends Platform {
    constructor(x, y, width, height) {
        super(x, y, width, height, 'HorizontalPlatform');
    }
...
// Sous-classe VerticalPlatform
class VerticalPlatform extends Platform {
    constructor(x, y, width, height) {
        super(x, y, width, height, 'VerticalPlatform');
    }
...
// Add a new method to change the platform type by name (used by button click)
Player.prototype.changePlatformTypeByName = function (type) {
    console.log('platform type change prototype')
    console.log('this.platformMessage')
    console.log(this.platformMessage)
    const platform = this.platformMessage;
    ...
    console.log('type :')
    console.log(type)
    console.log("platformClasses[type] :")
    console.log(platformClasses[type])

    let newPlatformClass = platformClasses[type];

    if (!newPlatformClass) {
        console.error(`Platform type ${type} is not defined.`);
        return;
    }
...
const levels = [
    {
        player: { x: 100, y: 300 },
        platforms: [
            { type: 'HorizontalPlatform', x: 50, y: 360, width: 100, height: 20},
        ],
        coins: [
    ...
    },
    {
        player: { x: 100, y: 250 },
        platforms: [
            { type: 'HorizontalPlatform', x: 50, y: 350, width: 100, height: 20},
            { type: 'VerticalPlatform', x: 550, y: 350, width: 100, height: 20}
        ],
        coins: [
            { x: 250, y: 270, radius: 10 }
        ],
        doors: [
            { x: 580, y: 100, width: 40, height: 50 }
        ],
        walls: [
        ],
    }
    ...
    levelData.platforms.forEach(data => {
        switch(data.type){
            case 'HorizontalPlatform':
                platforms.push(new HorizontalPlatform(data.x, data.y, data.width, data.height));
                break;
            case 'VerticalPlatform':
                platforms.push(new VerticalPlatform(data.x, data.y, data.width, data.height));
                break;
        }
    });
